...
{
    public class GradingDecoratingWorker<C>(IGradingWorker<C> original, Func<C, ILogging?, bool> filter, Func<IAsyncEnumerable<KeyValuePair<IGradingCriterion, IGradingResult>>, ILogging?, IAsyncEnumerable<KeyValuePair<IGradingCriterion, IGradingResult>>> applayeReselt, string? hashKey = null) : IGradingWorker<C>
        where C : IGradingCriterion
    {
        private readonly IGradingWorker<C> _original = original;
        ...
        {
            using var log = logging?.CreateChildLogging(typeof(GradingDecoratingWorker<>), this);
